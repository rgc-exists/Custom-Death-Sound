...
bool SearchFilterPopup::init(std::string query, bool recent, bool inUse, std::function<void(std::string, bool, bool)> onApply) {
    ...
    queryLabel->setPosition({ 34.f, 166.f });
    m_mainLayer->addChild(queryLabel);

    m_queryInput = TextInput::create(210.f, "Type to search...");
    m_queryInput->setPosition({ m_mainLayer->getContentWidth() / 2.f, 142.f });
    ...
    recentLabel->setPosition({ 34.f, 108.f });
    ...
    if (offSprite && onSprite) {
        ...
        m_recentToggle->setPosition({ m_mainLayer->getContentWidth() - 40.f, 108.f });
        ...
        inUseLabel->setPosition({ 34.f, 78.f });
        ...
        if (m_inUseToggle) {
            ...
            m_inUseToggle->setPosition({ m_mainLayer->getContentWidth() - 40.f, 78.f });
    ...
    clearBtn->setPosition({ m_mainLayer->getContentWidth() / 2.f - 60.f, 44.f });
    ...
    applyBtn->setPosition({ m_mainLayer->getContentWidth() / 2.f + 60.f, 44.f });
