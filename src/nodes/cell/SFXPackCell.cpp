...
void SFXPackCell::validateDownloadState() {
    ...
    log::info("Validating downloaded pack '{}' ({})", m_name, m_sfxId);
    for (auto const& soundId : m_soundIds) {
        auto path = resolveSoundPath(soundId);
        if (!std::filesystem::exists(path)) {
            log::warn(
                "Missing sound '{}' in pack '{}' ({}) at {}",
                soundId,
                m_name,
                m_sfxId,
...
void SFXPackCell::recomputePackStateFromSounds(bool forceStateUpdate) {
    ...
    for (auto const& soundId : m_soundIds) {
        ...
        if (std::filesystem::exists(path)) {
            ...
            log::info(
                "Missing file while recomputing pack '{}' ({}) sound '{}' -> {}",
                m_name,
                m_sfxId,
                soundId,
                geode::utils::string::pathToString(path)
...
void SFXPackCell::onDownloadToggle(CCObject* sender) {
    ...
    log::info(
        "Toggle pressed for '{}' ({}) state={} downloaded={} skipped={} failed={}",
        m_name,
        m_sfxId,
...
void SFXPackCell::onEditPressed(CCObject* sender) {
    EditSoundPackPopup::create(this)->show();
}

void SFXPackCell::onExit() {
    log::info("Exiting '{}' ({}) and cancelling in-flight tasks", m_name, m_sfxId);
...
void SFXPackCell::cancelPackDownload() {
    log::info("Cancel requested for '{}' ({})", m_name, m_sfxId);
    m_cancelRequested = true;
    m_sfxInfoTask.cancel();
    m_packDownloadTask.cancel();
...
void SFXPackCell::startSoundDownload(std::string const& soundId, std::string const& displayName, std::vector<std::string> candidates) {
    ...
    log::info(
        "Preparing sound '{}' ({}) with candidates {}",
        displayName,
        soundId,
    ...
    if (std::filesystem::exists(resolveSoundPath(soundId))) {
        log::info("Skipping '{}' ({}) because it already exists", displayName, soundId);
        ++m_skippedCount;
        notifyProgress(
...
void SFXPackCell::tryDownloadSound(std::string const& soundId, std::string const& displayName, std::vector<std::string> const& candidates, size_t candidateIndex) {
    ...
    m_packDownloadTask.spawn(
        ...
        [this, soundId, displayName, candidates, candidateIndex, alive_token, url](web::WebResponse value) {
            ...
            if (!value.ok()) {
                log::warn(
                    "HTTP failure for '{}' ({}) candidate {} code={} url={}",
                    displayName,
                    soundId,
                    candidateIndex,
                    value.code(),
...
void SFXPackCell::downloadNextSound() {
    ...
    if (m_downloadIndex >= m_soundIds.size()) {
        if (m_failedCount > 0) {
            log::warn(
                "Pack '{}' ({}) finished with failures: downloaded={} skipped={} failed={}",
                m_name,
                m_sfxId,
                m_downloadedCount,
                m_skippedCount,
            ...
            log::info(
                "Pack '{}' ({}) completed successfully: downloaded={} skipped={} failed={}",
                m_name,
                m_sfxId,
                m_downloadedCount,
                m_skippedCount,
    ...
    log::info(
        "Fetching metadata for pack '{}' ({}) sound '{}' from {}",
        m_name,
        m_sfxId,
        soundId,
        sfxInfoUrl
    ...
    m_sfxInfoTask.spawn(
        ...
        [this, soundId, alive_token](web::WebResponse response) {
            ...
            if (!response.ok()) {
                log::warn(
                    "Metadata request failed for '{}' ({}) HTTP {}",
                    m_name,
                    soundId,
                    response.code()
                );
            ...
            if (!json.isOk()) {
                log::error(
                    "Metadata response was not JSON for '{}' ({})",
                    m_name,
                    soundId
                );
                startSoundDownload(soundId, soundId, { makeSoundRelativeUrl(soundId) });
            ...
            log::info(
                "Metadata for pack '{}' ({}) sound '{}' resolved name='{}' candidates={}",
                m_name,
                m_sfxId,
                soundId,
                displayName,
