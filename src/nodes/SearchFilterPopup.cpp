...
bool SearchFilterPopup::init(std::string query, bool recent, std::function<void(std::string, bool)> onApply) {
    ...
    queryLabel->setPosition({ 34.f, 126.f });
    m_mainLayer->addChild(queryLabel);

    m_queryInput = geode::TextInput::create(210.f, "Type to search...");
    m_queryInput->setPosition({ m_mainLayer->getContentWidth() / 2.f, 104.f });
    m_queryInput->setCommonFilter(geode::CommonFilter::Any);
    ...
    recentLabel->setPosition({ 34.f, 72.f });
    ...
    if (offSprite && onSprite) {
        ...
        m_recentToggle->setPosition({ m_mainLayer->getContentWidth() - 45.f, 72.f });
    ...
    clearBtn->setPosition({ m_mainLayer->getContentWidth() / 2.f - 60.f, 28.f });
    ...
    applyBtn->setPosition({ m_mainLayer->getContentWidth() / 2.f + 60.f, 28.f });
...
SearchFilterPopup* SearchFilterPopup::create(std::string query, bool recent, std::function<void(std::string, bool)> onApply) {
    auto ret = new SearchFilterPopup();
    if (ret && ret->init(std::move(query), recent, std::move(onApply))) {
        ret->autorelease();
        return ret;
    }
